...
SBMP_seagrass = d %>% dplyr::filter(Park %in% c("SBMP"))
SBMP = SBMP_seagrass %>% dplyr::filter(Method %in% c("Posidonia transect/quadrats"))
SBMP_westerngulf = SBMP %>% dplyr::filter(Site %in% c("0380 Settlement", "Sandy Point",
                  "South Passage", "Useless Loop North", "Useless Loop South"))
SBMP_peron = SBMP %>% dplyr::filter(Site %in% c("Big Lagoon", "Denham", "Peron South"))
SBMP_monkeymia = SBMP %>% dplyr::filter(Site %in% c("Monkey Mia Inner Bank" ,
                "Monkey Mia Pearl Control", "East Peron" , "Monkey Mia South", "Monkey Mia south_outer"))
SBMP_wooramel = SBMP %>% dplyr::filter(Site %in% c("Wooramel North", "Disappointment Reach"))
...
make_shootdensity <- function(df){
  df %>%
    ...
    dplyr::summarise(
      ...
      se   = sd(Posidonia_australis, na.rm = TRUE) / sqrt(N)
    )
}

SBMP_shootdensity <- make_shootdensity(SBMP)
...
SBMP_westerngulf_shootdensity <- make_shootdensity(SBMP_westerngulf)
...
SBMP_westerngulf_shootdensity_plot <- ggplot(SBMP_westerngulf_shootdensity, aes(x=Year, y=mean)) +
  ...
  scale_x_continuous(breaks = seq(2010,2017,1), limits=c(min(2010), max(SBMP_westerngulf_shootdensity$Year+0.125))) +
  scale_y_continuous(limits=c(min(0), max(15)))+
  xlab("Year") +
  ylab(expression(paste("Mean (±SE) density (","0.04m"^-2,")", sep = ""))) +
...
SBMP_peron_shootdensity <- make_shootdensity(SBMP_peron)

SBMP_peron_shootdensity_plot<-ggplot(SBMP_peron_shootdensity, aes(x=Year, y=mean)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.02, colour="black", position=pd) +
  #geom_line(position=pd) +
  geom_point(position=pd, size=3, fill="black") + # 21 is filled circle
  scale_x_continuous(breaks = seq(2010,2017,1), limits=c(min(2010), max(SBMP_peron_shootdensity$Year+0.125))) +
  scale_y_continuous(limits=c(min(0), max(15)))+
  xlab("Year") +
  ylab(expression(paste("Mean (±SE) density (","0.04m"^-2,")", sep = ""))) +
  ggtitle("b) Peron")+
...
SBMP_monkeymia_shootdensity <- make_shootdensity(SBMP_monkeymia)

SBMP_monkeymia_shootdensity_plot <- ggplot(SBMP_monkeymia_shootdensity, aes(x=Year, y=mean)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.02, colour="black", position=pd) +
  #geom_line(position=pd) +
  geom_point(position=pd, size=3, fill="black") + # 21 is filled circle
  scale_x_continuous(breaks = seq(2010,2017,1), limits=c(min(2010), max(SBMP_monkeymia_shootdensity$Year+0.125))) +
...
SBMP_wooramel_shootdensity <- make_shootdensity(SBMP_wooramel)
...
#Overall max height
SBMP_maxheight <- make_maxheight(SBMP)

SBMP_maxheight_plot <- ggplot(SBMP_maxheight, aes(x=Year, y=mean)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.02, colour="black", position=pd) +
  #geom_line(position=pd) +
  geom_point(position=pd, size=3, fill="black") + # 21 is filled circle
  scale_x_continuous(breaks = seq(2010,2017,1), limits=c(min(2010), max(SBMP_maxheight$Year+0.125))) +
...
SBMP_westerngulf_maxheight <- make_maxheight(SBMP_westerngulf)

SBMP_westerngulf_maxheight_plot <- ggplot(SBMP_westerngulf_maxheight, aes(x=Year, y=mean)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.02, colour="black", position=pd)+
  #geom_line(position=pd) +
  geom_point(position=pd, size=3, fill="black")+
  scale_x_continuous(breaks = seq(2010,2017,1), limits=c(min(2010), max(SBMP_westerngulf_maxheight$Year+0.125))) +
...
SBMP_peron_maxheight <- make_maxheight(SBMP_peron)

SBMP_peron_maxheight_plot<-ggplot(SBMP_peron_maxheight, aes(x=Year, y=mean))+
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.02, colour="black", position=pd)+
  #geom_line(position=pd) +
  geom_point(position=pd, size=3, fill="black")+ # 21 is filled circle
  scale_x_continuous(breaks = seq(2010,2017,1), limits=c(min(2010), max(SBMP_peron_maxheight$Year+0.125))) +
  scale_y_continuous(limits=c(min(0), max(500)))+
  xlab("Year")+
  ylab(expression(paste("Mean (±SE) max. height (mm)", sep = ""))) +
  ggtitle("b) Peron")+
...
SBMP_monkeymia_maxheight <- make_maxheight(SBMP_monkeymia)

SBMP_monkeymia_maxheight_plot <- ggplot(SBMP_monkeymia_maxheight, aes(x=Year, y=mean)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.02, colour="black", position=pd) +
  #geom_line(position=pd) +
  geom_point(position=pd, size=3, fill="black") + # 21 is filled circle
  scale_x_continuous(breaks = seq(2010,2017,1), limits=c(min(2010), max(SBMP_monkeymia_maxheight$Year+0.125))) +
  scale_y_continuous(limits=c(min(0), max(500)))+
  xlab("Year") +
  ylab(expression(paste("Mean (±SE) max. height (mm)", sep = ""))) +
  ggtitle("c) Monkey Mia")+
...
SBMP_wooramel_maxheight <- make_maxheight(SBMP_wooramel)
...
# #Overall mean height
#
# SBMP_meanheight <- make_meanheight(SBMP)
#
...
#
# SBMP_westerngulf_meanheight <- make_meanheight(SBMP_westerngulf)
#
...
# #Peron mean height
#
# SBMP_peron_meanheight <- make_meanheight(SBMP_peron)
#
...
#
# SBMP_monkeymia_meanheight <- make_meanheight(SBMP_monkeymia)
#
...
#   xlab("Year") +
#   # ylab(expression(paste("Mean +/- SE) 80th percentile height (mm)", sep = ""))) +
#   ylab(bquote('Mean (±SE) 80th percentile height'))+
#   ggtitle("c) Monkey Mia")+
#   # geom_smooth(method=lm, colour = 1, linetype = 3, se=FALSE, fullrange=TRUE)+
#   theme_bw() + graphics+
#   theme(plot.title = element_text(size = 25))
...
#
# SBMP_wooramel_meanheight <- make_meanheight(SBMP_wooramel)
#
