setwd("~/projects/data-pipelines/scripts/indicators/seagrass/shoot_height")
...
pdf_rid <- "ce18973e-3336-466e-834b-e08abfeec5be"
txt_rid <- "b71decc7-90a0-4e62-88b0-8134f97a0258"
pdf_fn = "final.pdf"
...
SIMP_s_plot <- ggplot(d_sum, aes(x=Year, y=mean, group=Zone, linetype=Zone, shape=Zone)) +
  ...
  scale_x_continuous(limits=c(min(2012-0.125), max(d_sum$Year+0.125)), breaks=min(d_sum$Year):max(d_sum$Year)) +
  scale_y_continuous(limits=c(min(0), max(700)))+
  xlab("Year") +
  ylab(expression(paste("Mean maximum height (mm)", sep = ""))) +
  ggtitle("SIMP_south")+
  theme_bw() + graphics
...
SIMP_w_plot<-ggplot(d_sum_simpw, aes(x=Year, y=mean, group=Zone, linetype=Zone, shape=Zone)) +
  ...
  geom_line(position=pd) +
  geom_point(position=pd, size=3, fill="black") + # 21 is filled circle
  scale_x_continuous(limits=c(min(2012-0.125), max(d_sum_simpw$Year+0.125)), breaks=min(d_sum_simpw$Year):max(d_sum_simpw$Year)) +
  scale_y_continuous(limits=c(min(0), max(700)))+
  xlab("Year") +
  ylab(expression(paste("Mean maximum height (mm)", sep = ""))) +
  ggtitle("SIMP_Warnbro Sound")+
  theme_bw() + graphics
dev.off()
...
SIMP_shoal_plot <- ggplot(d_sum_shoal, aes(x=Year, y=mean, group=Zone, linetype=Zone, shape=Zone)) +
  ...
  geom_line(position=pd) +
  geom_point(position=pd, size=3, fill="black") + # 21 is filled circle
  scale_x_continuous(limits=c(min(2012-0.125), max(d_sum_shoal$Year+0.125)), breaks=min(d_sum_shoal$Year):max(d_sum_shoal$Year)) +
  scale_y_continuous(limits=c(min(0), max(700)))+
  xlab("Year") +
  ylab(expression(paste("Mean maximum height (mm)", sep = ""))) +
  ggtitle("SIMP_Shoalwater Bay")+
  theme_bw() + graphics
...
SIMP_n_plot<-ggplot(d_sum_simpn, aes(x=Year, y=mean, group=Zone, linetype=Zone, shape=Zone)) +
  ...
  geom_line(position=pd) +
  geom_point(position=pd, size=3, fill="black") + # 21 is filled circle
  scale_x_continuous(limits=c(min(2012$Year-0.125), max(d_sum_simpn$Year+0.125)), breaks=min(d_sum_simpn$Year):max(d_sum_simpn$Year)) +
  scale_y_continuous(limits=c(min(0), max(700)))+
  xlab("Year") +
  ylab(expression(paste("Mean maximum height (mm)", sep = ""))) +
  ggtitle("SIMP_north")+
