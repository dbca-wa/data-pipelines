...
setwd("T:/529-CALMscience/Shared Data/Marine Science Program/MONITORING/Pluto Offset 4 Monitoring/Assets/Seagrass/Precision Analysis 20182")
...
sg <-read.csv("DMP_SEAGRASS_2018_PRECISIONANALYSIS_Dot Point Measurements.csv", header=TRUE,fileEncoding="UTF-8-BOM") #note: this is for reading a CSV rather than excel spreadsheet
...
sg<-sg%>%
  ...
  dplyr::mutate(PointNo=cumsum(Frame))
...
write.csv(statsLv1DF, lvl1, col.names = TRUE, row.names = FALSE)


## DISPLAY OUTPUT ----
dir()
dat<- read.xlsx("StatsOutput_Level1.xlsx")
head(dat)
# *****
# LEVEL 1 CATEGORIES
