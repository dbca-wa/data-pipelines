...
####----  5.3 Add feeding guild and target species data----####
specinfo.complete.sumcount<- speciesinfo%>%
  select(-c(family,genus, species))%>%
    right_join(complete.sumcount, by="scientific")

specinfo.complete.length.number.mass<- speciesinfo%>%
  select(-c(family,genus, species))%>%
  right_join(complete.length.number.mass, by="scientific")

specinfo.complete.length.number<-speciesinfo%>%
  select(-c(family,genus, species))%>%
...
setwd(temp)
write.csv(missingfeeding, file=paste(study,"feeding.guilds.missing.csv",sep = "_"), row.names=FALSE )
...
####----  5.6 review the list of target species to ensure you are happy and it matches the most recent DPIRD reports----####
...
####----  5.9 Select columns out for merging ----####
names(metadata)
met<-metadata%>%
  setNames(tolower(names(.)))%>%
  select(c(location,opcode,latitude,longitude,year, site,depth))

####----  5.10 Merge in metadata ----####
complete.sumcount.meta<-specinfo.complete.sumcount%>%
  left_join(met, by="opcode")
head(complete.sumcount)

complete.length.number.mass.meta<-specinfo.complete.length.number.mass%>%
  left_join(met, by="opcode")

complete.length.number.meta<-specinfo.complete.length.number%>%
  left_join(met, by="opcode")

####----  5.11 ADDING IN HABITAT DATA WILL GO HERE----####

####----  5.12 WRITE  complete and expanded data----####
