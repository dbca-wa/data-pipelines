...
dat.ab.richness2 <- subset(dat.ab.richness, Year %in% c("2010", "2011", "2014", "2015"))
dat.ab.richness3 <- subset (dat.ab.richness2, Site %in% c("FishB1", "FishB2", "FishL1", "FishL2", "FishT1", "FishT2", "FishCB1", "FishCB2"))

#Obtain mean and SE values for species Richness at zone level

RichnessNMP <- ddply(dat.ab.richness3, .(Year, Zone), summarise,
                   N    = length(richness),
                   mean = mean(richness),
                   sd   = sd(richness),
