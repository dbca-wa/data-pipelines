...
dat.ab.target <- subset(dat.ab.total2, Genus.Species %in% c("Lethrinus nebulosus", "Lethrinus laticaudis", "Lethrinus miniatus", "Lethrinus punctulatus", "Epinephelus rivulatus", "Epinephelus multinotatus", "Epinephelus areolatus", "Epinephelus coioides", "Cephalopholis sonnerati", "Plectropomus spp", "Plectropomus leopardus", "Plectropomus maculatus", "Lutjanus argentimaculatus", "Lutjanus lemniscatus", "Rachycentron canadus"))
...
#y = dat.ab.target.mand$total
#x = as.integer(dat.ab.target.mand$Year)
#gamtarget.mand <- gam(data = dat.ab.target.mand, formula = y~x + poly(x,2) + poly(x,3), na.action = "na.fail", family = negbin(1))
#gamtarget.mand.compare <- dredge(trace = 1, gamtarget.mand, m.lim = c(0,1), extra = c("adjR^2","R^2","AIC", "BIC"))
...
TargetMandFig <- ggplot(TargetMand, aes(x=Year, y=mean, group=Zone, linetype=Zone, shape=Zone)) +
  ...
  #geom_line(position=pd) +                      # line
  geom_point(position=pd, size=5) +             # points
  #stat_smooth(method = "lm", colour = "black", se = FALSE) +
  xlab("Survey Year") +
  ylab(expression(paste("Abundance per 250 ", m^2, "", " +/- SE", sep = ""))) +
  ggtitle("Mandu (n = 6-8)") +
...
#y = dat.ab.target.osp$total
#x = as.integer(dat.ab.target.osp$Year)
#gamtarget.osp <- glm(data = dat.ab.target.osp, formula = y~poly(x,4), na.action = "na.fail", family = nb())
#gamtarget.osp.compare <- dredge(trace = 1, gamtarget.osp, m.lim = c(0,1), extra = c("adjR^2","R^2","AIC", "BIC"))
...
TargetOspFig <- ggplot(TargetOsp, aes(x=Year, y=mean, group=Zone, linetype=Zone, shape=Zone)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.25, position=pd) + # error bars
  #geom_smooth(method = "gam", formula = y ~ s(x,k=3), se=F, size = 0.5,col="black") +
  #geom_smooth(method = "lm", formula = y.pred$fit ~ x.pred, se=F, size = 0.5,col="grey") +
  #geom_line(position=pd) +                      # line
  geom_point(position=pd, size=5) +             # points
  #stat_smooth(method = "lm", colour = "black", se = FALSE) +
  xlab("Survey Year") +
...
#y = dat.ab.target.maud$total
#x = as.integer(dat.ab.target.maud$Year)
#gamtarget.maud <- gam(data = dat.ab.target.maud, formula = y~x + poly(x,2) + poly(x,3) + poly(x,4), na.action = "na.fail", family = negbin(1))
#gamtarget.maud.compare <- dredge(trace = 1, gamtarget.maud, m.lim = c(0,1), extra = c("adjR^2","R^2","AIC", "BIC"))
...
TargetMaudFig <- ggplot(TargetMaud, aes(x=Year, y=mean, group=Zone, linetype=Zone, shape=Zone)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.25, position=pd) + # error bars
  #geom_smooth(method = "gam", formula = y ~ s(x,k=3), se=F, size = 0.5,col="black") +
  #geom_smooth(method = "gam", formula = y ~ poly(x,2), se=F, size = 0.5,col="black") +
  #geom_line(position=pd) +                      # line
  geom_point(position=pd, size=5) +             # points
  #stat_smooth(method = "lm", colour = "black", se = FALSE) +
  xlab("Survey Year") +
